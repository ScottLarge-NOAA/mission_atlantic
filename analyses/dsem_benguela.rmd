...
# remotes::install_github("James-Thorson-NOAA/dsem@dev")
library(dsem) ## 1.3.0
library(phylopath)
...
sem_var <- function(data, sem, start_year) {
  
  sem_first <- unlist(stringr::str_extract_all(sem, "(?<=\\n).*?(?=->)"))
  sem_first <- gsub("\\s", "", sem_first)
  
  sem_second <- unlist(stringr::str_extract_all(sem, "(?<=->).*?(?=,)"))
  sem_second <- gsub("\\s", "", sem_second)
  sem_param <- unique(c(sem_first[!grepl("#.*", sem_first)], sem_second[!grepl("#.*", sem_first)]))
...
rps_wide <- logrps %>% 
  select(-rps) %>% 
  pivot_wider(names_from = species, values_from = c(biomass, log_rps,recruits))
```
...
-   Environmental Indicators
    ...
    -   Larger zooplankton may be more abundant when upwelling is stronger and smaller zooplankton may be more abundant when upwelling is weaker, which could suggest that sardine diet is negatively influenced by upwelling [@vanderlingen2006].
        -   Cumulative coastal South Benguela and Agulhas Bank upwelling from December to March [@lamont2018]. (upwelling_AB and upwelling_SB; see Figure \@ref(fig:upsummer-plot))
-   Ecosystem responses:
    -   Southern Benguela
        -   Mean chlorophyll a (mg m^-3^) from December to February (chla_SB) [@lamont2019]
        -   Mean fractional contribution of microphytoplankton (micro_SB; >20 μm), nanophytoplankton (nano_SB; 2-20 μm), and picophytoplankton (pico_SB; <2 μm) from December to February [@lamont2019]
    -   Agulhas Bank
        -   Mean chlorophyll *a* (mg m^-3^) from December to February (chla_AB) [@lamont2019]
        -   Mean fractional contribution of microphytoplankton (micro_AB; >20 μm), nanophytoplankton (nano_AB; 2-20 μm), and picophytoplankton (pico_AB; <2 μm) from December to February (chla_AB) [@lamont2019]
...
```{r, z-plot, fig.width=12, fig.height=12, fig.cap= "All indicators"}
zplot
```
...
Given the complexity of the system, we first established a subset of simple conceptual models to inform a full model (i.e., forward model selection).

1)  Environmental - Physical forcing may lead to environments that favor sardine or anchovy. Increase in zooplankton size will favor anchovy. In SB, larger zooplankton are more abundant with strong upwelling [@vanderlingen2006]

```{r sem-env-bs, echo=FALSE}
...
sem_env_bs <- "
  ...
  upwelling_SB -> pico_SB, 0, upwelling_to_picoSB
  upwelling_SB -> nano_SB, 0, upwelling_to_nanoSB
  upwelling_SB -> micro_SB, 0, upwelling_to_microSB
  sst_SB -> pico_SB, 0, sst_to_picoSB
  sst_SB -> nano_SB, 0, sst_to_nanoSB
  sst_SB -> micro_SB, 0, sst_to_microSB

  ## SB Trophic links between PP and zooplankton
  pico_SB -> smallzp_SB, 0, pico_to_smallzpSB
  nano_SB -> smallzp_SB, 0, nano_to_smallzpSB
  micro_SB -> mdlgzp_SB, 0, micro_to_largezpSB
  # chla_SB -> copepodabund_SB, 0, chla_to_copepodabundSB
  # chla_SB -> accs_SB, 0, chla_to_accsSB

  ## SB Trophic links between zoop and small pelagics
  smallzp_SB -> biomass_Sardine, 0, smallzp_to_BiomassS
  mdlgzp_SB -> biomass_Anchovy, 0, mdlgzp_to_BiomassS

  ### AR
  upwelling_SB -> upwelling_SB, 1, AR1, 0.001
  sst_SB -> sst_SB, 1, AR2, 0.001

  pico_SB -> pico_SB, 1, AR3, 0.001
  nano_SB -> nano_SB, 1, AR4, 0.001
  micro_SB -> micro_SB, 1, AR5, 0.001

  smallzp_SB -> smallzp_SB, 1, AR6, 0.001
  mdlgzp_SB -> mdlgzp_SB, 1, AR7, 0.001
  # accs_SB -> accs_SB, 1, AR14, 0.001
  # chla_SB -> chla_SB, 1,  AR10, 0.001
  # copepodabund_SB -> copepodabund_SB, 1, AR13, 0.001

  biomass_Anchovy -> biomass_Anchovy, 1, AR8, 0.001
  biomass_Sardine -> biomass_Sardine, 1, AR9, 0.001
#### ---
"
...
select_env_bs <-  c(
  ...
  "smallzp_SB -> smallzp_SB, 1, AR6, 0.001",
  "mdlgzp_SB -> mdlgzp_SB, 1, AR7, 0.001",
  # "accs_SB -> accs_SB, 1, NA_AR8, 0.001",
  # "chla_SB -> chla_SB, 1,  NA_AR9, 0.001",
  # "copepodabund_SB -> NA_copepodabund_SB, 1, AR10, 0.001",
  "biomass_Anchovy -> biomass_Anchovy, 1, AR11, 0.001",
  "biomass_Sardine -> biomass_Sardine, 1, AR12, 0.001"
  #### ---
)
sem_dat_env_bs <- sem_var(data = sem_dat, sem = sem_env_bs, start_year = 1979)
all_var_env_bs <- colnames(sem_dat_env_bs)

# Fit dsem with stepwise selection
select_fit_env_bs <- stepwise_selection(model_options = select_env_bs,
                                   model_shared = "",
                                   tsdata = ts(sem_dat_env_bs),
                                   family =  rep('fixed', ncol(sem_dat_env_bs)),
                                   control = dsem_control(use_REML = TRUE, quiet=TRUE),
                                   quiet = FALSE)

# Fit dsem with stepwise selection
select_fit_env_bs <- dsem(sem = sem_env_bs,
                          # model_shared = "",
                          tsdata = ts(sem_dat_env_bs),
                          family =  rep('fixed', ncol(sem_dat_env_bs)),
                          control = dsem_control(use_REML = TRUE, quiet=FALSE))
AIC(select_fit_env_bs)

select_dat_env_bs <- sem_var(data = sem_dat, sem = paste0("\n", select_fit_env_bs$model), start_year = 1979)

fit_env_bs <- dsem(sem = select_fit_env_bs$model,
                   tsdata = ts(select_dat_env_bs),
                   family =  rep('fixed', ncol(select_dat_env_bs)),
                   control = dsem_control(use_REML = TRUE, quiet=FALSE))

plot(fit_env_bs, edge_label = "value_and_stars", style = "ggraph")

fit_select_env_bs <- suppressWarnings(dsem(sem = select_env_bs$model,
                                           tsdata = ts(sem_dat_env_bs),
                                           family =  rep('fixed', ncol(sem_dat_env_bs)),
                                           control = dsem_control(use_REML = TRUE, quiet=FALSE)))
TMBAIC(fit_select_env_bs$opt)

summary_env_bs <- summary(fit_env_bs) %>% 
  mutate(sig = cut(p_value, breaks = c(-Inf, 0.01, 0.05, 0.10, Inf), 
                   labels = c("***", "**", "*", "NS")))

ParHat_simple = fit_env_bs$obj$env$parList()

## 
# TMBAIC(fit_env_bs$opt)
# summary(fit_env_bs)
...
# Fit dsem
fit_fish <- suppressWarnings(dsem( sem = sem_fish,
...
```{r dsem-full-dag, fig.height = 12, fig.width = 15}
...
# Extract stuff
out_fish <- summary(fit_fish)
# Format inputs
from_fish <- ifelse(out_fish[,2] == 0, 
               out_fish$first, 
               paste0("lag(", out_fish$first, ", ", out_fish[,2],")"))
vertices_fish <- union(out_fish$second, from_fish)
fish_df <- data.frame(from = from_fish, 
                       to = out_fish$second, 
                       name = out_fish[,3]) 

fish_df$label <- round(out_fish$Estimate, digits = 2)

add_stars_fish <- cut(out_fish[,'p_value'], breaks=c(1,0.05,0.01,0.001,0) )
add_stars_fish <- c("***","**","*","")[as.numeric(add_stars_fish)]
fish_df$label = paste0(fish_df$label, add_stars_fish)
fish_df$color = case_when(!grepl("\\*", fish_df$label) ~ "grey40",
                     fish_df$label > 0 ~ "blue",
                     fish_df$label < 0 ~ "red",
                     TRUE ~ NA_character_)
fish_df <- fish_df %>% 
  mutate(label = round(out_fish$Estimate, digits = 2),
         label = paste0(label, add_stars_fish),
         color = case_when(!grepl("\\*", label) ~ "grey40",
                     label > 0 ~ "blue",
                     label < 0 ~ "red",
                     TRUE ~ NA_character_)) %>% 
  filter(!grepl("^V\\[.*", name))
...
# pg <- plot( fit_simple, edge_label = "value_and_stars", style="ggraph")

pg_fish <- igraph::graph_from_data_frame( d = fish_df,
                               directed = TRUE,
                               vertices = data.frame(vertices_fish) )
...
# flip_x = FALSE
# flip_y = FALSE
l_fish <- ggraph::create_layout(pg_fish, layout = 'igraph', algorithm = 'sugiyama')
l_fish$y <- ifelse(grepl(x = l_fish$name, pattern = "^lag\\("), 
                   purrr::map_dbl(.x = l_fish$y, ~ round(runif(1, min = -.25, max = 2) + .x, 2)),
                   l_fish$y)

arrow <- grid::arrow(type = 'closed', 18, grid::unit(15, 'points'))
p1_fish <- ggraph::ggraph(l_fish) +
  ggraph::geom_edge_arc(aes(label = label, edge_colour = color), 
                        angle_calc = "across", force_flip = TRUE, 
                        strength = 0, arrow = arrow, edge_width = 1, 
                        # check_overlap = TRUE,
                        label_push = grid::unit(-0.75, "cm"),
                        end_cap = ggraph::rectangle(box_x, box_y, 'mm'),
                        start_cap = ggraph::rectangle(box_x, box_y, 'mm')) +
  ggraph::geom_node_text(ggplot2::aes(label = name), hjust = 0) +
...
```{r full-plot, fig.height=15, fig.width=12}
# for(i in lags_fish){
#   p1_fish <- NULL
#   p2_fish <- NULL
#   
...
# Fit dsem
fit_simple <- suppressWarnings(dsem( sem = sem_simple,
...
```{r dsem_simple_dag, fig.height = 8, fig.width = 15}
...
# Extract stuff
out <- summary(fit_simple)
# Format inputs
from <- ifelse(out[,2] == 0, 
               out$first, 
               paste0("lag(", out$first, ", ", out[,2],")"))
vertices <- union(out$second, from )
simple_df <- data.frame(from = from, 
                       to = out$second, 
                       name = out[,3]) 

simple_df$label <- round(out$Estimate, digits = 2)

add_stars <- cut(out[,'p_value'], breaks=c(1,0.05,0.01,0.001,0) )
add_stars <- c("***","**","*","")[as.numeric(add_stars)]
simple_df$label = paste0(simple_df$label, add_stars)
simple_df$color = case_when(!grepl("\\*", simple_df$label) ~ "grey40",
                     simple_df$label > 0 ~ "blue",
                     simple_df$label < 0 ~ "red",
                     TRUE ~ NA_character_)
simple_df <- simple_df %>% 
  mutate(label = round(out$Estimate, digits = 2),
         label = paste0(label, add_stars),
         color = case_when(!grepl("\\*", label) ~ "grey40",
                     label > 0 ~ "blue",
                     label < 0 ~ "red",
                     TRUE ~ NA_character_)) %>% 
...
pg <- igraph::graph_from_data_frame( d = simple_df,
                               directed = TRUE,
...
# flip_x = FALSE
# flip_y = FALSE
l = ggraph::create_layout(pg, 'igraph', algorithm = 'sugiyama')

arrow <- grid::arrow(type = 'closed', 18, grid::unit(15, 'points'))
p1_simple <- ggraph::ggraph(l) +
  ggraph::geom_edge_arc(aes(label = label, edge_colour = color), 
                        angle_calc = "across", force_flip = TRUE, 
                        strength = 0, arrow = arrow, edge_width = 1, 
                        label_push = grid::unit(-0.75, "cm"),
